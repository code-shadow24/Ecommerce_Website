...
const createShipment = asyncHandler(async(req, res)=>{
    ...
    if(!shipment){
        throw new ApiError(500, "Error occured while creating shipment")
    }
    ...
    if(!emailSent){
        throw new ApiError(500, "Error occured while sending email")
    }
...
const cancelShipment = asyncHandler(async(req, res)=>{
    const {transactionObjectId} = req.params.transactionObjectId || req.body || req.query

    if(!transactionObjectId){
        throw new ApiError(400, "Transaction Object Id is required")
    }

    const cancellation = await shippo.Transaction$.cancel(transactionObjectId)

    if(!cancellation){
        throw new ApiError(500, "Error occured while cancelling the shipment")
    }
...
const trackShipment = asyncHandler(async(req, res)=>{
    const {trackingNumber} = req.body || req.query || req.params.trackingNumber

    if(!trackingNumber){
        throw new ApiError(400, "Tracking number is required")
    }

    const trackingInfo = await shippo.Track$.getTrackingInfo(trackingNumber)
...
const retrieveShippingLabel = asyncHandler(async(req, res)=>{
    ...
    const shippingLabel = await shippo.Transaction$.getLabel(transactionObjectId)
...
const validateShippingAddress = asyncHandler(async(req, res)=>{
    ...
    const validAddress = await shippo.utils.validateAddress(address);

    if(!validAddress){
        throw new ApiError(500, "Error occured while validating address")
    }
...
const createReturnShipment = asyncHandler(async(req, res)=>{
    const {originalTransactionId} = req.body || req.params.originalTransactionId || req.query

    if(!originalTransactionId){
        throw new ApiError(400, "Original transaction id is required")
    }

    const returnShipment = await shippo.Transaction$.createForReturn(originalTransactionId)
